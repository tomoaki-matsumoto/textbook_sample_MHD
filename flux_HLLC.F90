...
contains
  ...
  !-----------------------------------------------------------------------
  ! get numerical flux in one dimension (Toro 2009, Riemann Solvers and Numerical Method for Fluid Dynamics, p.331)
  !-----------------------------------------------------------------------
...
#define SWR(A, B)  (0.5d0 - 0.5d0*sign(1.d0, (A))*sign(1.d0, (B)))
  ...
  subroutine flux( ql, qr, f, ndir)
    ...
    real(kind=DBL_KIND) :: &
         ...
         ul1,ul2,ul3,ul4,ul5, &
         ur1,ur2,ur3,ur4,ur5 ,&
         ual1,ual2,ual3,ual4,ual5, &
         uar1,uar2,uar3,uar4,uar5 ,&
         gm1, rhobar, csbar, ppvrs, swp, &
         sr, sl, swl ,swal, swar, swr, swt, rhobarl, rhobarr, cbar, pa, qal, qar, sa
    call util_arroffset(ndir,io,jo,ko)
    do k = KMIN-ko, KMAX
!$omp parallel do private(i,gm1,rhol,ul,vl,wl,pl,el,hl,cl,rhor,ur,vr,wr,pr,er,hr,cr,ul1,ul2,ul3,ul4,ul5,fl1,fl2,fl3,fl4,fl5,ur1,ur2,ur3,ur4,ur5,fr1,fr2,fr3,fr4,fr5,rhobar,cbar,ppvrs,pa,swp,qal,sl,qar,sr,sa,rhobarl,ual1,ual2,ual3,ual4,ual5,fal1,fal2,fal3,fal4,fal5,rhobarr,uar1,uar2,uar3,uar4,uar5,far1,far2,far3,far4,far5,swl,swal,swar,swr,swt)
       do j = JMIN-jo, JMAX
          do i = IMIN-io, IMAX
             gm1 = GAMMA - 1
             ! --------------
             ...
             fr5 = fr1*hr
             ! p*
             rhobar = (rhol + rhor)*0.5d0
             cbar = (cl + cr)*0.5d0
             ppvrs = (pl + pr)*0.5d0 - (ur - ul)*rhobar*cbar*0.5d0
             pa = max(0.d0, ppvrs)
             swp = SWD(pa, pl)
             qal = swp * sqrt(1.d0 + (GAMMA+1.d0)/(2.d0*GAMMA)*(pa/pl-1.d0)) + (1.d0-swp)
             sl = ul - cl * qal
             swp = SWD(pa, pr)
             qar = swp * sqrt(1.d0 + (GAMMA+1.d0)/(2.d0*GAMMA)*(pa/pr-1.d0)) + (1.d0-swp)
             sr = ur + cr * qar
!!$             sl = min(ul-cl, ur-cr)
!!$             sr = max(ul+cl, ur+cr)

             ! speed
             sa = ((sr - ur)*rhor*ur - (sl - ul)*rhol*ul - pr + pl)/((sr-ur)*rhor - (sl-ul)*rhol)

             ! HLL flux left
             rhobarl = rhol * (sl - ul)/(sl - sa)
             ual1 = rhobarl
             ual2 = rhobarl * sa
             ual3 = rhobarl * vl
             ual4 = rhobarl * wl
             ual5 = rhobarl * (el/rhol + (sa - ul)*(sa + pl/(rhol*(sl - ul))))
             fal1 = fl1 + sl * (ual1 - ul1)
             fal2 = fl2 + sl * (ual2 - ul2)
             fal3 = fl3 + sl * (ual3 - ul3)
             fal4 = fl4 + sl * (ual4 - ul4)
             fal5 = fl5 + sl * (ual5 - ul5)
             ! HLL flux right
             rhobarr = rhor * (sr - ur)/(sr - sa)
             uar1 = rhobarr
             uar2 = rhobarr * sa
             uar3 = rhobarr * vr
             uar4 = rhobarr * wr
             uar5 = rhobarr * (er/rhor + (sa - ur)*(sa + pr/(rhor*(sr - ur))))
             far1 = fr1 + sr * (uar1 - ur1)
             far2 = fr2 + sr * (uar2 - ur2)
             far3 = fr3 + sr * (uar3 - ur3)
             far4 = fr4 + sr * (uar4 - ur4)
             far5 = fr5 + sr * (uar5 - ur5)
             ! switch
             swl = SWD(sl, 0.d0)
             swal = SWR(sl, sa)
             swar = SWR(sa, sr)
             swr = SWD(0.d0, sr)
             swt = 1.d0/(swl + swal + swar + swr)
             ! -----------------
             ! flux
             ! -----------------
             f(i,j,k,MRHO) = (swl*fl1 + swal*fal1 + swar*far1 + swr*fr1)*swt
             f(i,j,k,MVX)  = (swl*fl2 + swal*fal2 + swar*far2 + swr*fr2)*swt
             f(i,j,k,MVY)  = (swl*fl3 + swal*fal3 + swar*far3 + swr*fr3)*swt
             f(i,j,k,MVZ)  = (swl*fl4 + swal*fal4 + swar*far4 + swr*fr4)*swt
             f(i,j,k,MP)   = (swl*fl5 + swal*fal5 + swar*far5 + swr*fr5)*swt
