...
for n = 0, n_elements(files)-1 do begin
   ...
   error[n] = total(abs(B-ex))
   dtime[n] = Time
endfor
plot, dtime, error, /xlog, /ylog
oplot, dtime, error, psym=4
oplot,[min(dtime),max(dtime)], (10000*[min(dtime),max(dtime)])^2
oplot,[min(dtime),max(dtime)], (10000*[min(dtime),max(dtime)])
end
